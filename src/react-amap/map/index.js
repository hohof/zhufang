...
var StaticProps = ['view', 'zooms', 'showIndoorMap', 'indoorMap', 'expandZoomRange', 'showBuildingBlock', 'viewMode', 'pitchEnable', 'buildingAnimation', 'skyColor', 'mask'];
...
var BaseMap = function (_Component) {
  ...
  function BaseMap(props) {
    ...
    if (typeof window !== 'undefined') {
      ...
      new _APILoader2.default({
        ...
        return _this.getAreaMask();
      }).then(function (res) {
        _this.createInstance(res);
  ...
  _createClass(BaseMap, [{
    ...
    key: 'createInstance',
    value: function createInstance(res) {
      if (!this.map) {
        var options = this.buildCreateOptions(res);
    ...
    value: function getAreaMask() {
      ...
      return new Promise(function (resolve) {
        // 利用行政区查询获取边界构建mask路径
        // 也可以直接通过经纬度构建mask路径
        if (!_this3.props.maskName) {
          resolve({});
          return;
        }
        district.search(_this3.props.maskName, function (status, result) {
          if (status === 'complete') {
    ...
    value: function buildCreateOptions(res) {
      var _this4 = this;
      ...
      CreateProps.forEach(function (key) {
        if (key in props) {
          options[key] = _this4.getSetterValue(key, props);
        }
      });
      return _extends({}, options, res);
    }
  }, {
    key: 'updateMapProps',
    value: function updateMapProps(prevProps, nextProps) {
      ...
      StatusDynamicProps.forEach(function (key) {
        if (key in nextProps) {
          ...
          if (_this5.detectPropChanged(key, prevProps, nextProps)) {
    ...
    value: function removeOrDisablePlugins(plugins) {
      ...
      if (plugins && plugins.length) {
        plugins.forEach(function (p) {
          if (p in _this7.pluginMap) {
            // ControlBar has no 'hide' method
            if (p === 'ControlBar') {
              _this7.map.removeControl(_this7.pluginMap[p]);
              delete _this7.pluginMap[p];
            } else {
              _this7.pluginMap[p].hide();
            }
          }
        });
      }
